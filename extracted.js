...
function switchView(v){
  ...
  if(['staff','grafikEditor','timeline','reports','logs'].includes(v)&&!isAdmin){
    toast(currentLang === 'DE' ? 'Dieser Tab ist nur für Administratoren verfügbar' : (currentLang === 'UA' ? 'Ця вкладка доступна тільки для адміністраторів' : 'Ta zakładka jest dostępna tylko dla administratora')); v='main';
  }
...
function renderReports(entries) {
  ...
  let html = `
    ...
    <div class="card">
      ...
      ${Object.keys(driverStats).sort((a,b)=>driverStats[b].kg-driverStats[a].kg).map(drv => `
...
function loadWeek(){
  ...
  google.script.run.withFailureHandler(function(err){alert(t('error_label') + ': '+(err&&err.message?err.message:'Nieznany błąd'));}).withSuccessHandler(function(data){entries=data||[];renderGrid();}).getEntriesForWeeks([weekKey(weekOffset-1),weekKey(weekOffset),weekKey(weekOffset+1)]);
}
...
function cancelComment() {
  ...
  pendingToggleEntries = null;
}

function confirmComment() {
  const commentText = document.getElementById('commentInput').value.trim();
  document.getElementById('commentModal').style.display = 'none';
  if (pendingToggleEntries) {
    performToggle(pendingToggleEntries, commentText);
    pendingToggleEntries = null;
  }
}
  ...
  tBtn.onclick=function(){
    ...
    // Jeśli cofamy odbiór, zapytaj o komentarz
    if (allDone) {
      pendingToggleEntries = displayEntries;
      document.getElementById('commentInput').value = displayEntries[0].comment || '';
...
google.script.run
  .withFailureHandler(function(err){
    ...
    alert(alertMsg + translateError(err.message));
  })
  .withSuccessHandler(function(data){
