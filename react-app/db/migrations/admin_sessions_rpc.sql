...
begin
  ...
  from (
    select
      ...
      count(s.id) filter (where s.revoked_at is null and s.expires_at > now())::integer as active_sessions,
      min(s.created_at) filter (where s.revoked_at is null and s.expires_at > now()) as oldest_active_at,
      max(s.created_at) filter (where s.revoked_at is null and s.expires_at > now()) as newest_active_at,
      max(s.last_seen_at) filter (where s.revoked_at is null and s.expires_at > now()) as last_seen_at,
      max(s.expires_at) filter (where s.revoked_at is null and s.expires_at > now()) as latest_expires_at
    from public.users u
    left join public.user_sessions s on s.user_id = u.id
    group by u.id, u.username, u.name, u.role, u.password_hash
    order by
      count(s.id) filter (where s.revoked_at is null and s.expires_at > now()) desc,
      u.role,
      u.username
  ) x;
...
begin
  ...
  v_pruned := public.prune_user_sessions(null, greatest(coalesce(p_keep_active, 3), 1));
